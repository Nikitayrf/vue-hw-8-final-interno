<template>
  <section class="page3">
    ...
    <div class="btn-menu">
      <BtnComponent v-for="room in rooms" :colorBtn="colorBtn" :handler="clickTagFilter" :key="room.id">{{ room }}
      </BtnComponent>
    </div>
    <div class="projects">
      <ul>
        <ProjectComponent v-for="project in projects" :project="project" :key="project.id" />
      </ul>
      <div class="btn-menu">
        <BtnComponent v-if="showBtn" :colorBtn="colorBtn" :handler="prevPage">
          Prev
        </BtnComponent>
        <BtnComponent v-if="showBtn" :colorBtn="colorBtn" :handler="nextPage">
          Next
        </BtnComponent>
      </div>
    </div>
